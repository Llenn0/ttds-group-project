...
class CloudDoc:
    def __init__(self, index_api: firestore.CollectionReference, token_id: int,
        ...
        if cloud_dict is None:
            cloud_dict = self.index_api.document(self.doc_name[:-3]).get(timeout=30).to_dict()
    ...
    def __contains__(self, i: int) -> bool:
        ...
        if self.is_segmented:
            ...
            if best_guess in self.accessed_slice:
                return False
            guessed_slice = self.index_api.document(self.doc_name %(best_guess)).get(timeout=30).to_dict()
    ...
    def cache_slices(self, slices_to_alloc: set[int]):
        ...
        if slices_to_alloc:
            ...
            for i in range(num_batches):
                ...
                for doc_ref in self.index_api.where("__name__", "in", value=names_to_fetch).limit(len(slices_to_alloc)).stream(timeout=30):
    ...
    def __getitem__(self, i: int):
        ...
        if self.is_segmented:
            ...
            fetched_slice = self.index_api.document(self.doc_name + '_' + str(slice_id)).get(timeout=30).to_dict()
            if 'd' in fetched_slice:
                for k, v in zip(fetched_slice['h'], fetched_slice['d']):
                    self.positions_cache[k] = np.array(json.loads(v), dtype=np.uint32)
            else:
...
class CloudIndexDict(dict):
    ...
    def __missing__(self, key: int|tuple[int]) -> CloudDoc|list[CloudDoc]:
        if isinstance(key, int):
            ...
            if str_keys:
                for doc_ref in self.index_api.where("__name__", "in", value=str_keys).limit(len(key)).stream(timeout=30):
                    k = int(doc_ref.id)
